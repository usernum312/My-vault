...
Conversation title:`;
  ...
  // Load sessions: prefer dedicated conversations.json; fall back to legacy data.json (migration)
  let savedSessions = await this.loadConversations();
  if (!savedSessions) {
    // First run after update — migrate sessions from the old data.json location
    const legacyData = await this.loadData();
    if (legacyData?.sessions?.length) {
      savedSessions = legacyData.sessions;
      // Sessions will be written to conversations.json on the next saveState call
    }
  }
  this._sessionManager = (savedSessions && savedSessions.length)
    ? new SessionManager(savedSessions)
    : new SessionManager();
...
@keyframes ai-pulse {
  ...
  /**
   * Persist the current sessions (excluding temporary ones) to conversations.json.
   * This keeps session data out of data.json, which is reserved for settings only.
   */
  async saveConversations() {
    ...
    try {
      await this.app.vault.adapter.write(
        ...
        JSON.stringify(nonTemporary, null, 2)
      );
    } catch (e) {
  ...
  /**
   * Load sessions from conversations.json.
   * Returns null if the file doesn't exist yet (first run / migration needed).
   */
  async loadConversations() {
    try {
      ...
      return null; // File not found or parse error — caller handles migration
    }
  }
