...
// ---------- TASK COMPLETION (Binary Only) ----------
function getTaskCompletion(page, searchTerms) {
    if (!page.file.tasks) return false;
...
const data = items.map(item => {
    ...
    allPages.forEach(p => {
        ...
        if (getTaskCompletion(p, item.terms)) {
    ...
    for (let page of allPages) {
        ...
        if (d && d.isValid() && d.isSame(today, 'day')) {
            todayComplete = getTaskCompletion(page, item.terms);
            break;
        }
    }
...
// ---------- WARNING ----------
const todayIncompleteCount = data.filter(d => !d.todayComplete).length;
...
if (todayIncompleteCount > 1) {
    const warning = dv.el("div", "تحذير: لديك أكثر من صلاة غير مكتملة اليوم", {
        attr: { style: `font-weight:900;color:#BD2626;font-size:1.2em;margin-bottom:10px;text-align:center;animation: glowBlur 1.5s ease-in-out infinite;text-shadow: 0 0 10px #ff0000, 0 0 20px #000;` }
    });
...
data.forEach((item, i) => {
    ...
    ctx.fillText(`${item.done}/${item.target}`, x, y + 15);
    ...
    if (!item.todayComplete) {
        ...
        ctx.fillStyle = item.streak > 2 ? "#10b981" : "#ca8a04";
    }
...
const items = [
    { icon: "🌅", label: "Morning", terms: ["Morning Athkar", "أذكار الصباح"], target: 3 },
    { icon: "🌇", label: "Evening", terms: ["Evening Athkar", "أذكار المساء"], target: 3 },
    { icon: "🌌", label: "Bedtime", terms: ["Bedtime Athkar", "أذكار النوم"], target: 3 },
];
