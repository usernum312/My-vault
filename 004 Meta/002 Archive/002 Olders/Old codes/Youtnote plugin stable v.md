...
var t=Ui(e);this.videos=t.videos,this.notes=t.notes,this.activeVideoId&&!this.videos.find(e=>e.id===this.activeVideoId)&&(this.activeVideoId=null),!this.activeVideoId&&this.videos.length>0&&(this.activeVideoId=this.videos[0].id),this.render();if(typeof this._onViewDataChanged==='function')this._onViewDataChanged();}clear(){this.videos=[],this.notes=[],this.activeVideoId=null,this.render()}onOpen(){this.contentEl.empty();this.root=rn(this.contentEl);this._btnExport=this.addAction(`file-down`,`Export as Markdown`,()=>{(async()=>{if(!this.file)return;let e=Ki(this.videos,this.notes),t=`${this.file.basename} - Export`;await this.createExportFile(t,e)})()});this._btnMarkdown=this.addAction(`file-text`,`Open as Markdown`,()=>{this.plugin.youtnoteFileModes[this.leaf.id??this.file?.path??``]=`markdown`,this.plugin.setMarkdownView(this.leaf)});this.applyHeaderButtonVisibility();this.render();return Promise.resolve()}onClose(){return this.root?.unmount(),Promise.resolve()}applyHeaderButtonVisibility(){
    ...
    if (this.plugin.settings.autoSyncTranscript) {
        ...
        if (activeVideo?.url) {
            const clean = URLDetector.cleanYouTubeUrl(activeVideo.url);
            if (URLDetector.isValidYouTubeUrl(clean)) {
...
class UnifiedPlugin extends import_obsidian.Plugin {
        ...
        this.addCommand({
            ...
            callback: async () => {
                ...
                modal.openAndGetValue(async (rawUrl) => {
                    ...
                    if (!URLDetector.isValidYouTubeUrl(url)) { new import_obsidian.Notice('Invalid YouTube URL'); return; }
                    try {
            ...
            if (leaf?.view?.getViewType?.() === Ji) {
                ...
                if (this.settings.autoSyncTranscript) {
                    ...
                    if (file) {
                            ...
                            try {
                                ...
                                if (cached && typeof cached === 'string') {
                                    ...
                                    cached.then(text => {
                                        ...
                                        if (u) {
                                            const c = URLDetector.cleanYouTubeUrl(u);
                                            if (URLDetector.isValidYouTubeUrl(c)) this.forceSidebarTranscript(c);
                                        }
                                    }).catch(()=>{});
                                }
                            } catch(_e) {}
                        ...
                        if (rawUrl) {
                            const clean = URLDetector.cleanYouTubeUrl(rawUrl);
                            if (URLDetector.isValidYouTubeUrl(clean)) this.forceSidebarTranscript(clean);
                        }
                    }
                }
            } else {
    ...
    forceSidebarTranscript(url) {
        const clean = URLDetector.cleanYouTubeUrl(url);
        if (!clean) return;
    ...
    _openTranscriptForActiveYoutnoteVideo() {
        ...
        const clean = URLDetector.cleanYouTubeUrl(rawUrl);
        if (!URLDetector.isValidYouTubeUrl(clean)) { new import_obsidian.Notice('Invalid YouTube URL'); return; }
                ...
                view._onViewDataChanged = function() {
                    ...
                    if (plugin.settings.autoSyncTranscript) {
                        ...
                        if (file) {
                            ...
                            if (rawUrl) {
                                const clean = URLDetector.cleanYouTubeUrl(rawUrl);
                                if (URLDetector.isValidYouTubeUrl(clean)) plugin.forceSidebarTranscript(clean);
                            } else if (!rawUrl) {
                                ...
                                try {
                                    ...
                                    if (cached && typeof cached.then === 'function') {
                                        cached.then(text => {
                                            ...
                                            if (u) {
                                                const c = URLDetector.cleanYouTubeUrl(u);
                                                if (URLDetector.isValidYouTubeUrl(c)) plugin.forceSidebarTranscript(c);
                                            }
                                        }).catch(()=>{});
                                    }
                                } catch(_e) {}
