...
class TranscriptView extends import_obsidian.ItemView {
    ...
    _startAutoScroll() {
        this._stopAutoScroll();
        // Use rAF loop instead of setInterval so scroll is smooth
        // regardless of playback speed.
        let lastScrollMs = -1;
        const loop = () => {
            this._rafHandle = requestAnimationFrame(loop);
            // Throttle actual work to ~4 fps (250ms) — sufficient for
            // transcript blocks which are multi-second each.
            const now = performance.now();
            if (now - lastScrollMs < 250) return;
            lastScrollMs = now;
    ...
    _tickAutoScroll() {
        ...
        const youtnoteLeaves = this.plugin.app.workspace.getLeavesOfType(Ji);
        ...
        // Add a lookahead proportional to playback rate so the highlight
        // stays ahead of the spoken word at 1.5x, 2x, etc.
        const rate = playerRef.cachedPlaybackRate ?? 1;
        const lookaheadMs = Math.max(0, (rate - 1) * 1500); // e.g. 1500ms ahead at 2x
    ...
    async renderTranscriptBlocks(url, data, timestampMod, searchTerm = '') {
        ...
        const blockEls = []; // [{offsetMs, el}] for auto-scroll
        ...
        // Store block elements for auto-scroll
    ...
    async setEphemeralState(state) {
        ...
        // URL comes directly from state — no leafUrls storage needed
        const url = state.url ? URLDetector.cleanYouTubeUrl(state.url) : null;
        if (!url) return;
        // Skip reload only if this exact URL is already fully loaded
        if (this.isDataLoaded && this._loadedUrl === url) return;
        this.isDataLoaded = false;
        this._loadedUrl = url;
        this._stopAutoScroll();
        // Clear all previous content so the new transcript renders cleanly
        this.contentEl.empty();
        this.contentEl.createEl('h4', { text: 'Transcript' });
...
};handleUpdateVideos=e=>{this.videos=e,this.render(),this.requestSave()};handleUpdateNotes=e=>{this.notes=e,this.render(),this.requestSave()};handleSetActiveVideoId=e=>{
...
class UnifiedSettingTab extends import_obsidian.PluginSettingTab {
    ...
    display() {
        ...
        new import_obsidian.Setting(containerEl)
            .setName('Auto-sync transcript on video switch')
            .setDesc('Automatically open/refresh the transcript sidebar when you select a different video in the Youtnote workspace.')
            .addToggle(t => t.setValue(this.plugin.settings.autoSyncTranscript)
                .onChange(async v => { this.plugin.settings.autoSyncTranscript = v; await this.plugin.saveSettings(); }));

        new import_obsidian.Setting(containerEl)
            .setName('Clear transcript when leaving workspace')
            .setDesc('Automatically close the transcript sidebar whenever you leave a Youtnote workspace.')
            .addToggle(t => t.setValue(this.plugin.settings.clearTranscriptOnLeave)
                .onChange(async v => { this.plugin.settings.clearTranscriptOnLeave = v; await this.plugin.saveSettings(); }));
        ...
        const yn = (name, desc, key) => new import_obsidian.Setting(containerEl).setName(name).setDesc(desc)
        ...
        yn('Show export button', 'Show the "Export as Markdown" button in the note header.', 'showExportButton');
        yn('Show open as Markdown button', 'Show the "Open as Markdown" button in the note header.', 'showMarkdownButton');
        yn('Show transcript button', 'Show the "Open transcript" button in the note header.', 'showTranscriptButton');

        containerEl.createEl('h4', { text: 'Workspace buttons' });
        yn('Show export all videos button', 'Show the button that exports all videos\' notes inside the workspace.', 'showExportAllButton');
        yn('Show export selected video button', 'Show the button that exports the selected video\' notes inside the workspace.', 'showExportVideoButton');
        yn('Show merge duplicates button', 'Show the button that merges notes with the same timestamp inside the workspace.', 'showMergeDuplicatesButton');

        containerEl.createEl('h4', { text: 'Workspace buttons' });
        yn('Show export all videos button', 'Show the button that exports all videos\' notes as Markdown.', 'showExportAllButton');
        yn('Show export selected video button', 'Show the button that exports the selected video\'s notes as Markdown.', 'showExportVideoButton');
        yn('Show merge duplicates button', 'Show the button that merges notes with the same timestamp.', 'showMergeDuplicatesButton');
        ...
        yn('Persist expanded state', 'Remember which notes are expanded when switching videos.', 'persistExpandedState');
        yn('Open exported file', 'Automatically open the exported Markdown file after creation.', 'openExportedFile');
        yn('Show note statistics', 'Display word count and character count in the note list header.', 'showNoteStats');
        yn('Pin video on phone (sticky)', 'Keep the video player visible at the top while scrolling on mobile.', 'pinOnPhone');
        ...
        new import_obsidian.Setting(containerEl)
            ...
            .addText(t => t.setPlaceholder('Enter API key').setValue(this.plugin.settings.apiKey || '')
        ...
        new import_obsidian.Setting(containerEl)
            ...
            .addText(t => t.setPlaceholder(DEFAULT_TRANSCRIPT_FOLDER).setValue(this.plugin.settings.transcriptFolder)
        ...
        new import_obsidian.Setting(containerEl).setName('Show search bar')
        ...
        new import_obsidian.Setting(containerEl).setName('Show copy all button')
        ...
        new import_obsidian.Setting(containerEl).setName('Show create note button')
        ...
        new import_obsidian.Setting(containerEl)
            ...
            .addText(t => t.setValue(this.plugin.settings.timestampMod.toString())
                .onChange(async v => { const n = parseInt(v); this.plugin.settings.timestampMod = isNaN(n) ? DEFAULT_TIMESTAMP_MOD : n; await this.plugin.saveSettings(); }));

        new import_obsidian.Setting(containerEl).setName('Language')
...
class UnifiedPlugin extends import_obsidian.Plugin {
    ...
    async onload() {
        ...
        Yi.prototype.getViewData = function() {
            ...
            const adapter = this._playerAdapterRef;            const position = Math.floor(adapter?.cachedCurrentTime ?? 0);
        ...
        this.addCommand({
            ...
            callback: async () => {
                ...
                modal.openAndGetValue(async (rawUrl) => {
                    ...
                    if (!url) { new import_obsidian.Notice('Invalid YouTube URL — could not extract video ID.'); return; }
                    try {
                        ...
                        const existing = this.app.vault.getAbstractFileByPath(fileName);
                        if (existing) {
                            new import_obsidian.Notice(`Transcript already exists: ${fileName}`);
                            await this.app.workspace.getLeaf(false).openFile(existing);
                        } else {
                            const file = await this.app.vault.create(fileName, content);
                    ...
                    } catch (err) { new import_obsidian.Notice(`Error: ${err.message || 'Failed'}`); }
                });
            }
        });
        ...
        this.addCommand({
            ...
            callback: async () => {
                ...
                this.youtnoteFileModes[leaf.id ?? created.path] = Ji;
        ...
        this.addCommand({
            ...
            callback: () => {
                ...
                if (leaf && leaf.view.getViewType() === 'markdown') {
                    this.youtnoteFileModes[leaf.id ?? leaf.view.file?.path ?? ''] = Ji;
        ...
        this.registerEvent(this.app.workspace.on('active-leaf-change', (leaf) => {
            ...
            if (_prevYoutnoteLeaf && _prevYoutnoteLeaf !== leaf) {
                ...
                // Clear transcript only when truly leaving the Youtnote context —
                // NOT when clicking the transcript sidebar itself or any other sidebar panel
                const newType = leaf?.view?.getViewType?.();
                const isTranscriptLeaf = newType === VIEW_TYPE_YTRANSCRIPT;
                const isYoutnoteLeaf   = newType === Ji;
                if (this.settings.clearTranscriptOnLeave && !isTranscriptLeaf && !isYoutnoteLeaf) {
            ...
            if (leaf?.view?.getViewType?.() === Ji) {
                ...
                // Auto-sync transcript — prefer active video URL, fall back to first frontmatter URL
                if (this.settings.autoSyncTranscript) {
                    const file = view.file;
                    if (file) {
                        // 1. Active video in workspace (best for multi-link notes)
                        const activeVideoUrl = view.videos?.find(v => v.id === view.activeVideoId)?.url;
                        // 2. First frontmatter link source entry
                        const fm = this.app.metadataCache.getFileCache(file)?.frontmatter;
                        const fmRaw = fm?.['link source'] || fm?.['link_source'];
                        const fmUrl = _unwrapLinkSourceUrl(fmRaw);
                        // 3. Scan cached file content (no await needed) for non-standard notes
                        let bodyUrl = null;
        ...
        // ---- Add "Open Transcript" action button to every Youtnote view ----
        ...
        // ---- Youtnote: inject toolbar button into markdown views for .youtnote files ----
        let mdScheduled = false, mdFrame = null;
        const scheduleMd = () => {
            if (!mdScheduled) {
                mdScheduled = true;
                mdFrame = window.requestAnimationFrame(() => {
                    mdScheduled = false; mdFrame = null;
                    this.app.workspace.iterateAllLeaves(leaf => {
                        ...
                        if (isYN) {
                            view.youtnoteActionEl = view.addAction('youtnote', 'Open as Youtnote view', () => {
                                this.youtnoteFileModes[leaf.id ?? file.path] = Ji;
                                this.setYoutnoteView(leaf);
                            });
        ...
        scheduleMd();
        this.register(() => { if (mdFrame !== null) { activeWindow.cancelAnimationFrame(mdFrame); mdFrame = null; mdScheduled = false; } });

        // ---- Youtnote: file context menu ----
        this.registerEvent(this.app.workspace.on('file-menu', (menu, file) => {
            (async () => {
                if (file instanceof import_obsidian.TFile && file.extension === 'md' && await this.isYoutnoteFile(file)) {
                    menu.addItem(item => {
                        item.setTitle('Open as Youtnote view').setIcon('youtnote').setSection('pane').onClick(() => {
                            const leaves = this.app.workspace.getLeavesOfType('markdown');
                            for (const l of leaves) {
                                if (l.view.file?.path === file.path) {
                                    this.youtnoteFileModes[l.id ?? file.path] = Ji;
                                    this.setYoutnoteView(l);
                                    return;
                                }
                            }
                            this.app.workspace.getLeaf(true).setViewState({ type: Ji, state: { file: file.path }, active: true });
                        });
                    });
                }
            })();
        ...
        // ---- Transcript: auto-extract on file modify ----
        ...
        this.registerEvent(this.app.metadataCache.on('changed', (file) => {
            ...
            if (activeLeaf.view?.getViewType?.() !== Ji) return;
        ...
        // We patch its prototype to register itself on the owning Yi view.
        const plugin = this;
        const origHandleMessage = pi.prototype.handleMessage;
        const origLoadVideo = pi.prototype.loadVideo;
        // Patch loadVideo to apply a pending seek AFTER the video is confirmed loaded,
        // avoiding the race condition where a seek fires before buffering completes.
        ...
        // Patch handleMessage to register the adapter on the owning Yi view,
        // and stage any pending seek/rate before origHandleMessage executes.
    ...
    forceSidebarTranscript(url) {
        // Normalise to watch?v= form so embed/youtu.be URLs all work
        const clean = URLDetector.toWatchUrl(url) || URLDetector.cleanYouTubeUrl(url);
        if (!clean) return;
        // Debounce: only suppress duplicate calls for the SAME URL within 500ms.
        // Different URLs always go through immediately (e.g. switching between multi-source videos).
        ...
        // Always reuse the single transcript leaf; close any extras
        ...
        leaf.setViewState({ type: VIEW_TYPE_YTRANSCRIPT }).then(() => {
            ...
            // setEphemeralState now handles URL-based caching internally
    ...
    async _insertTranscriptInActiveNote(url) {
        ...
        if (!view) { new import_obsidian.Notice('No active note found'); return; }
        const transcript = await YoutubeTranscript.fetchTranscript(url, { lang: this.settings.lang, country: this.settings.country });
        const formatted = TranscriptFormatter.format(transcript, url, { template: TEMPLATE_RICH, timestampMod: this.settings.timestampMod, showChapters: this.settings.showChapters });
    ...
    async insertTranscriptUnderLink(editor) {
        ...
        if (!url) { new import_obsidian.Notice('No YouTube URL found in selection'); return; }
        const transcript = await YoutubeTranscript.fetchTranscript(url, { lang: this.settings.lang, country: this.settings.country });
        const formatted = TranscriptFormatter.format(transcript, url, { template: TEMPLATE_RICH, timestampMod: this.settings.timestampMod, showChapters: this.settings.showChapters });
        const cursor = editor.getCursor();
    ...
    async _createTranscriptNotesForLinks(file, scriptLinks) {
        ...
        for (const link of scriptLinks) {
            try {
                ...
                const existing = this.app.vault.getAbstractFileByPath(fileName);
                if (!existing) {
                    const c = TranscriptFormatter.format(transcript, link.url, { template: TEMPLATE_RICH, timestampMod: this.settings.timestampMod, showChapters: this.settings.showChapters });
    ...
    refreshAllViews() {
        this.app.workspace.getLeavesOfType(Ji).forEach(leaf => {
            const v = leaf.view;
            if (v instanceof Yi && v.root) {
                ...
                // Apply header button visibility (export / markdown / transcript)
    ...
    monkeyPatchLeafSetViewState = () => {
        const plugin = this;
        const origSetViewState = import_obsidian.WorkspaceLeaf.prototype.setViewState;
        const origDetach = import_obsidian.WorkspaceLeaf.prototype.detach;

        import_obsidian.WorkspaceLeaf.prototype.setViewState = function(state, eState) {
            ...
            if (filePath && filePath.length > 0 && leafId && state.type === 'markdown' &&
                ...
                return origSetViewState.call(this, { ...state, type: Ji }, eState);
            }
            return origSetViewState.call(this, state, eState);
        };

        import_obsidian.WorkspaceLeaf.prototype.detach = function() {
        ...
        return () => {
            import_obsidian.WorkspaceLeaf.prototype.setViewState = origSetViewState;
            import_obsidian.WorkspaceLeaf.prototype.detach = origDetach;
        };
    };
    ...
    _openTranscriptForActiveYoutnoteVideo() {
        ...
        if (leaf?.view?.getViewType() !== Ji) {
            new import_obsidian.Notice('Open a Youtnote file first');
            return;
        }
        const view = leaf.view;
        const file = view.file;
        // Prefer active video URL (correct for multi-link notes); fall back to first frontmatter URL
        const activeVideoUrl = view.videos?.find(v => v.id === view.activeVideoId)?.url;
        const fm = file ? this.app.metadataCache.getFileCache(file)?.frontmatter : null;
        const fmRaw = fm?.['link source'] || fm?.['link_source'];
        const rawUrl = activeVideoUrl || _unwrapLinkSourceUrl(fmRaw);
        if (!rawUrl) { new import_obsidian.Notice('No video URL found. Make sure a video is loaded.'); return; }
        const clean = URLDetector.toWatchUrl(rawUrl);
        if (!clean) { new import_obsidian.Notice('Invalid YouTube URL — could not extract video ID.'); return; }
    ...
    _ensureTranscriptButtonsOnYoutnoteViews() {
        this.app.workspace.getLeavesOfType(Ji).forEach(leaf => {
            const view = leaf.view;
            if (view && !view._transcriptBtnAdded) {
                view._btnTranscript = view.addAction('scroll', 'Open transcript for this video', () => {
                    ...
                    if (rawUrl) {
                        const clean = URLDetector.toWatchUrl(rawUrl);
                        if (clean) this.forceSidebarTranscript(clean);
                        else new import_obsidian.Notice('Invalid YouTube URL');
                    } else {
                        new import_obsidian.Notice('No video URL found. Make sure a video is loaded.');
                    }
                });
                view._transcriptBtnAdded = true;
                if (typeof view.applyHeaderButtonVisibility === 'function') view.applyHeaderButtonVisibility();

                // Hook: called by setViewData whenever the note changes in this leaf
                const plugin = this;
                view._onViewDataChanged = function() {
                    const v = this;
                    // Clear transcript if enabled (note changed = new video context)
                    if (plugin.settings.clearTranscriptOnLeave) {
                        plugin.app.workspace.getLeavesOfType(VIEW_TYPE_YTRANSCRIPT)
                            .forEach(l => l.detach());
                    }
                    // Auto-sync transcript to the new note's active video
                    if (plugin.settings.autoSyncTranscript) {
                        ...
                        if (file) {
                            ...
                            const fmRaw = fm?.['link source'] || fm?.['link_source'];
                            const fmUrl = _unwrapLinkSourceUrl(fmRaw);
                            const rawUrl = activeVideoUrl || fmUrl;
                            if (rawUrl) {
                                const clean = URLDetector.toWatchUrl(rawUrl);
                                if (clean) plugin.forceSidebarTranscript(clean);
                            } else if (!rawUrl) {
                                // Non-standard note: scan cached content
                                try {
