...
function Ui(e){
    ...
    for(const u of urlList){
        const vid=Mi(u);
        if(vid) fmVideos.push({id:crypto.randomUUID(),url:u,title:``,durationSec:0,thumbnail:`https://img.youtube.com/vi/${vid}/default.jpg`});
    }
    ...
    for(let i=0;i<r.length;i++){
        ...
        if(fmVideos.length===0){
            ...
            if(ll&&ll[2]!==`timestamp`){
                ...
                if(vid){
                    finalizeNote();
                    curVideo={id:crypto.randomUUID(),title:ll[1],url:ll[2],durationSec:0,thumbnail:`https://img.youtube.com/vi/${vid}/default.jpg`};
        ...
        if(ts&&curVideo){
            ...
            curNote={id:crypto.randomUUID(),videoId:curVideo.id,timestampSec:secs,bodyMarkdown:``,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
            continue;
        }
...
class UnifiedPlugin extends import_obsidian.Plugin {
            ...
            if (leaf?.view?.getViewType?.() === Ji) {
                ...
                if (this.settings.autoSyncTranscript) {
                    ...
                    if (file) {
                        ...
                        const rawUrl = fm?.['link source'] || fm?.['link_source']
                            || view.videos?.find(v => v.id === view.activeVideoId)?.url;
                        if (rawUrl) {
    ...
    forceSidebarTranscript(url) {
        // Debounce: ignore calls with the same URL within 500ms
        ...
        // Reset so the view reloads for a new URL
        if (leaf.view instanceof TranscriptView && leaf.view._loadedUrl !== clean) {
            leaf.view.isDataLoaded = false;
        }
    ...
    refreshAllViews() {
        this.app.workspace.getLeavesOfType(Ji).forEach(leaf => {
            if (leaf.view instanceof Yi) leaf.view.refresh();
        });
    }
    ...
    _openTranscriptForActiveYoutnoteVideo() {
        ...
        const view = leaf.view;
        const file = view.file;
        // Prefer frontmatter "link source"; fall back to in-memory video URL
        const fm = file ? this.app.metadataCache.getFileCache(file)?.frontmatter : null;
        const rawUrl = fm?.['link source'] || fm?.['link_source']
            || view.videos?.find(v => v.id === view.activeVideoId)?.url;
        if (!rawUrl) { new import_obsidian.Notice('No video URL found. Add "link source: <url>" to frontmatter.'); return; }
        const clean = URLDetector.cleanYouTubeUrl(rawUrl);
    ...
    _ensureTranscriptButtonsOnYoutnoteViews() {
        this.app.workspace.getLeavesOfType(Ji).forEach(leaf => {
            ...
            if (view && !view._transcriptBtnAdded) {
                view.addAction('scroll', 'Open transcript for this video', () => {
                    const file = view.file;
                    const fm = file ? this.app.metadataCache.getFileCache(file)?.frontmatter : null;
                    const rawUrl = fm?.['link source'] || fm?.['link_source']
                        || view.videos?.find(v => v.id === view.activeVideoId)?.url;
                    if (rawUrl) {
                        ...
                        new import_obsidian.Notice('No video URL found in frontmatter.');
                    }
                });
