...
class UnifiedPlugin extends import_obsidian.Plugin {
    ...
    async onload() {
        ...
        this.registerEvent(this.app.workspace.on('active-leaf-change', (leaf) => {
            ...
            if (leaf?.view?.getViewType() !== Ji) return;
            const view = leaf.view;
            const video = view.videos?.find(v => v.id === view.activeVideoId);
            if (video?.url) {
                const clean = URLDetector.cleanYouTubeUrl(video.url);
    ...
    _openTranscriptForActiveYoutnoteVideo() {
        ...
        const view = leaf.view;
        const video = view.videos?.find(v => v.id === view.activeVideoId);
        if (!video?.url) { new import_obsidian.Notice('No video selected in the Youtnote workspace'); return; }
        const clean = URLDetector.cleanYouTubeUrl(video.url);
            ...
            if (view && !view._transcriptBtnAdded) {
                view.addAction('scroll', 'Open transcript for this video', () => {
                    ...
                    if (video?.url) {
                        ...
                        new import_obsidian.Notice('No video selected');
                    }
                });
