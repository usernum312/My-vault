...
// Replaces {{token}} (case-insensitive) with provided values.
// Unknown tokens are left as-is so the user can see them.
// ====================================================================
...
class TranscriptView extends import_obsidian.ItemView {
    ...
    renderHeader(url, data, timestampMod) {
        ...
        // ---- Create Timed Note button (sticky, top of sidebar) ----
        if (this.plugin.settings.showSidebarTimedNoteButton !== false) {
            ...
            (0, import_obsidian.setIcon)(timedNoteBtn, 'clock-plus');
            timedNoteBtn.createSpan({ text: 'Create Timed Note' });
            timedNoteBtn.addEventListener('click', () => {
                const leaves = this.plugin.app.workspace.getLeavesOfType(Ji);
                if (!leaves.length) {
                    new import_obsidian.Notice('Open a Youtnote workspace first');
                    return;
                }
                const view = leaves[0].view;
                if (view && typeof view._triggerCreateTimedNote === 'function') {
                    view._triggerCreateTimedNote();
                } else {
                    new import_obsidian.Notice('No active Youtnote workspace found');
                }
            });
            // Insert sticky before the header bar
...
const DEFAULT_SETTINGS = {
    ...
    // --- User-editable note templates ---
    // Transcript note (rich, with timestamps) — saved as a file
...
{{transcript_body}}`,
    // Transcript note — minimal (no timestamps, plain text)
...
#### Transcript
{{transcript_body}}`,
    // Youtnote export — per-video block header (single or multi-video)
...
class UnifiedSettingTab extends import_obsidian.PluginSettingTab {
    ...
    display() {
        ...
        yn('Show timed notes', 'Show or hide timed notes in the workspace notes panel. Can also be toggled via the command palette.', 'showTimedNotes');
        yn('Show "Create Timed Note" button in transcript sidebar', 'Show the sticky button at the top of the transcript sidebar for quickly adding timed notes.', 'showSidebarTimedNoteButton');
        yn('Show "Add note" button in workspace', 'Show the + button at the bottom of the notes pane for adding timed notes.', 'showWorkspaceTimedNoteButton');
        ...
        // Helper: create a textarea-based setting with a variable legend
        const addTemplateField = (name, desc, settingKey, defaultValue, variables) => {
            ...
            textarea.style.cssText = 'width:100%;font-family:var(--font-monospace);font-size:12px;resize:vertical;padding:8px;box-sizing:border-box;border-radius:4px;border:1px solid var(--background-modifier-border);background:var(--background-primary);color:var(--text-normal);';
            ...
            variables.forEach(([token, meaning]) => {
...
class UnifiedPlugin extends import_obsidian.Plugin {
    ...
    async onload() {
        ...
        this.addCommand({
            ...
            callback: () => {
                const leaves = this.app.workspace.getLeavesOfType(Ji);
                if (!leaves.length) {
                    new import_obsidian.Notice('Open a Youtnote workspace first');
                    return;
                }
        ...
        this.addCommand({
            ...
            callback: async () => {
                ...
                new import_obsidian.Notice(`Timed notes are now ${state}`);
            }
        });
    ...
    async _insertTranscriptInActiveNote(url) {
        ...
        const transcript = await YoutubeTranscript.fetchTranscript(url, { lang: this.settings.lang, country: this.settings.country });
        const formatted = TranscriptFormatter.format(transcript, url, { template: TEMPLATE_RICH, timestampMod: this.settings.timestampMod, showChapters: this.settings.showChapters, noteTemplate: this.settings.transcriptNoteTemplate });
    ...
    async _createTranscriptNotesForLinks(file, scriptLinks) {
        ...
        for (const link of scriptLinks) {
            try {
                ...
                const existing = this.app.vault.getAbstractFileByPath(fileName);
                if (!existing) {
                    const c = TranscriptFormatter.format(transcript, link.url, { template: TEMPLATE_RICH, timestampMod: this.settings.timestampMod, showChapters: this.settings.showChapters, noteTemplate: this.settings.transcriptNoteTemplate });
