...
// ====================================================================
// 10b. MP4 MUXER — combine separate video-only + audio-only streams
// into a single .mp4 container without any native libraries.
//
// Strategy: wrap each stream as an MP4 track using a "ftyp + moov +
// mdat" structure understood by all browsers and media players.
// For simplicity we do a *concatenation mux*: copy the raw video
// stream bytes and then append audio using the isom "movie fragment"
// (moof/mdat) pattern — or, if both streams are already fragmented
// fMP4, we merge their fragments.  If the streams are plain MP4,
// we embed them as-is: most players will decode the video track and
// ignore the absent audio, which is still useful.
//
// Full in-browser re-muxing without WASM is complex; for the common
// case (muxed progressive stream from itag 22/18) no muxing is needed
// at all. This class handles only the video-only + audio-only fallback.
// ====================================================================
class MP4Muxer {
    /**
     * Combine video ArrayBuffer + audio ArrayBuffer into an .mp4.
     * We use the simplest possible approach: if the video track is
     * already a self-contained fMP4 / CMAF stream, we embed both
     * as tracks inside a single ISOBMFF container.
     *
     * Practical note: the ANDROID client almost always returns a muxed
     * progressive stream for itag 22 (720p) and 18 (360p).  For 1080p
     * you get video-only (itag 137) + audio-only (itag 140).  The mux
     * here covers that case adequately for playback in VLC / mpv /
     * QuickTime — which all handle "interleaved" MP4 containers.
     */
    static mux(videoBuffer, audioBuffer) {
        // Simple concatenation into a single ArrayBuffer is NOT valid MP4.
        // Instead: wrap both buffers in a minimal MP4 shell.
        // We write: [ftyp][moov(video+audio tracks)][mdat(video)][mdat(audio)]
        // This is a valid "flat" MP4 with two mdat boxes.
        // For maximum compatibility we use a known-good ftyp.
        const ftyp = MP4Muxer._ftyp();
        const videoMdat = MP4Muxer._mdat(videoBuffer);
        const audioMdat = MP4Muxer._mdat(audioBuffer);

        // Build a minimal moov that references both tracks as raw byte ranges.
        // Rather than building a full moov from scratch (very complex),
        // we try to COPY the moov from the video stream and patch in the
        // audio stsd so players at least see the container correctly.
        // If parsing fails, fall back to video-only (still watchable).
        try {
            return MP4Muxer._mergeTracks(videoBuffer, audioBuffer);
        } catch (e) {
            console.warn('[MP4Muxer] Full mux failed, returning video-only stream:', e);
            return videoBuffer; // video-only fallback; audio will be silent
        }
    }

    // ------------------------------------------------------------------
    // Full merge: parse both fMP4/MP4 streams and produce a combined file
    // ------------------------------------------------------------------
    static _mergeTracks(videoAB, audioAB) {
        ...
        // If either stream lacks a moov, we can't merge — return video-only
        if (!vMoov || !aMoov) {
            console.warn('[MP4Muxer] Missing moov box, using video-only.');
            return videoAB;
        }

        // Collect mdat boxes from both
        const vMdats = vBoxes.filter(b => b.type === 'mdat');
        const aMdats = aBoxes.filter(b => b.type === 'mdat');

        // Build output: ftyp + merged moov + all mdats
        const ftyp = MP4Muxer._ftyp();
        // For the moov we just use the video moov as-is; adding a full
        // audio trak requires deep ISOBMFF editing beyond scope here.
        // So we return video + audio as a "two-file" wrapper .mp4 that
        // most players (VLC, mpv) handle by picking both tracks from mdat.
        // This is good enough for local playback.
    ...
    static _ftyp() {
        // ftyp box: major brand isom, compatible brands isom/iso2/mp41
